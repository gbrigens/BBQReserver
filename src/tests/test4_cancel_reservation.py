...
cancel_reservation_parameters = [
    ["🗑 Cancel reservation", "Choose the number of reservation you want to cancel:\n1. Date:2019-12-30 Time:16:00\n", {'id': 1234, 'first_name': 'Test', 'last_name': 'The Bot'}],
]


@pytest.mark.parametrize("test_input, expected, userdata", cancel_reservation_parameters)
def test_cancel_reservation(test_input, expected, userdata):
    bot = Mockbot()
    ug = UserGenerator()
    cg = ChatGenerator()
